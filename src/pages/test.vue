...
export default {
	...
	methods: {
		...
		async fetchPosts() {
			await this.handleChildSelection();

		// Fetch gender only if it's not already set
		if (!this.gender) {
		...
		// Fetch global data for charts if gender is defined
		if (this.gender) {
		...
		// Initialize charts based on the active sub-tab
